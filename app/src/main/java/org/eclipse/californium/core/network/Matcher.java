...
//				LOGGER.log(Level.FINE, "Exchange completed: Cleaning up {0}", idByToken);
                ...
                //if request is multicast, don't remove from deduplicator, unless there is too much time between request and response
				if(!exchange.getRequest().isMulticast()  ||   exchange.getResponse().getTimestamp() - exchange.getRequest().getTimestamp() > 20000 ){
